...
<div class="max-w-7xl mx-auto">
    ...
    <header class="mb-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-800 uppercase">Tambah Arsip Skripsi</h1>
            <p class="mt-1 text-sm text-slate-500 font-medium italic">Daftarkan karya ilmiah mahasiswa baru ke dalam sistem.</p>
        </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                    ...
                    <p class="mt-3 text-[10px] font-bold text-slate-400 leading-relaxed italic">
                        * Gunakan format JPG/PNG, maksimal 2MB.
                    </p>
                </div>
            </div>
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-8 py-5 border-b border-slate-100 bg-slate-50/50">
                        <h3 class="text-sm font-black text-slate-700 uppercase tracking-widest">Informasi Akademik Mahasiswa</h3>
                    </div>
                    ...
                    <div class="p-8 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="nama_penulis" value="{{ old('nama_penulis') }}" placeholder="Masukkan Nama Mahasiswa" required
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">NPM</label>
                                <input type="text" name="npm" value="{{ old('npm') }}" placeholder="Contoh: 2021001..." required
                                       class="w-full bg-slate-50 border border-slate-200 px-4 py-3 text-sm font-semibold rounded-xl focus:bg-white focus:border-slate-800 focus:ring-0 transition-all outline-none">
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Judul Skripsi / Tugas Akhir</label>
                                <textarea name="judul" rows="3" required placeholder="Tuliskan judul lengkap penelitian..."
                                          class="w-full bg-slate-50 border border-slate-200 px-4 py-3 text-sm font-semibold rounded-xl focus:bg-white focus:border-slate-800 focus:ring-0 transition-all outline-none resize-none tracking-tight leading-relaxed">{{ old('judul') }}</textarea>
                            </div>

                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Dosen Pembimbing</label>
                                <input type="text" name="dosen_pembimbing" value="{{ old('dosen_pembimbing') }}" placeholder="Nama Dosen Lengkap" required
                                       class="w-full bg-slate-50 border border-slate-200 px-4 py-3 text-sm font-semibold rounded-xl focus:bg-white focus:border-slate-800 focus:ring-0 transition-all outline-none">
                            </div>

                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tahun Lulus</label>
                                <input type="number" name="tahun" value="{{ old('tahun', date('Y')) }}" placeholder="2026" required
                                       class="w-full bg-slate-50 border border-slate-200 px-4 py-3 text-sm font-semibold rounded-xl focus:bg-white focus:border-slate-800 focus:ring-0 transition-all outline-none">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Fakultas</label>
                                <input type="text" x-model="selectedFakultas" readonly placeholder="Otomatis"
                        ...
                        <div class="pt-6 border-t border-slate-100">
                            ...
                            <textarea name="abstrak" rows="8" placeholder="Masukkan abstrak ringkasan penelitian..."
                                      class="w-full bg-slate-50 border border-slate-200 p-5 text-sm font-medium leading-relaxed rounded-2xl focus:bg-white focus:border-slate-800 focus:ring-0 transition-all outline-none resize-none tracking-tight">{{ old('abstrak') }}</textarea>
                        </div>
                    </div>
                </div>
